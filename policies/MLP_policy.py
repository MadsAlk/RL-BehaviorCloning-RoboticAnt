...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        ...
        means = self.mean_net(torch.tensor(observation, dtype=torch.float32))
    ...
    def update(self, observations, actions):
        ...
        dists = self.forward(observations)
        loss = -dists.log_prob(torch.tensor(actions, dtype=torch.float32)).sum()
