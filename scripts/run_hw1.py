...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            # print("PATHS:", len(paths), len(paths[0]['action'][0]), len(paths[0]['next_observation']))
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          # for imitation learning, we only need observations and actions.  
            ...
            # Use np.random.choice to sample indices for both lists
            sampled_indices = np.random.choice(len(replay_buffer.obs), size=batch_size, replace=False)

            # Use the sampled indices to retrieve corresponding observations and actions
            ob_batch = [replay_buffer.obs[i] for i in sampled_indices]
            ac_batch = [replay_buffer.acs[i] for i in sampled_indices]
